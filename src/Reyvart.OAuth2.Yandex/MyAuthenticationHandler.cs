    ...
    public class MyAuthenticationHandler : AuthenticationHandler<MyAuthenticationOptions>
    {
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
                ...
                {
                    context.Identity.AddClaim(new Claim("urn:yandex:name", context.FullName, XmlSchemaString, Options.AuthenticationType));
                }
                if (!string.IsNullOrEmpty(context.PhotoLink))
                {
                    context.Identity.AddClaim(new Claim("urn:yandex:link", context.PhotoLink, XmlSchemaString, Options.AuthenticationType));
                }
